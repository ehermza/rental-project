"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createPagoService = exports.getPagosByClientService = exports.getPagosService = void 0;
const Pago_1 = __importDefault(require("../models/Pago"));
function getPagosService() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield Pago_1.default.find();
        }
        catch (error) {
            throw Error(error);
        }
    });
}
exports.getPagosService = getPagosService;
function getPagosByClientService(idClient, nCtner) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // return await Pago.find();
            const filter = {
                'client': idClient,
                'id_container': nCtner
            };
            return yield Pago_1.default.find(filter);
        }
        catch (error) {
            throw Error(error);
        }
    });
}
exports.getPagosByClientService = getPagosByClientService;
function createPagoService(objpago) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield objpago.save();
        }
        catch (error) {
            throw Error(error);
        }
    });
}
exports.createPagoService = createPagoService;
